#include <sys/types.h>
#include <sys/socket.h>
#include <sys/time.h>
#include <sys/stat.h>
#include <netinet/in.h>
#include <errno.h>
#include <fcntl.h>
#include <unistd.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <signal.h>
#include <arpa/inet.h>
#include <string.h>
#include <pthread.h>

/* portul folosit */
#define PORT 2909

/* codul de eroare returnat de anumite apeluri */
extern int errno;

static void *treat(void *); /* functia executata de fiecare thread ce realizeaza comunicarea cu clientii */
//void raspunde(void *);

typedef struct {
	pthread_t idThread; //id-ul thread-ului
	int thCount; //nr de conexiuni servite
	FILE* fdTokens;
}Thread;

Thread *threadsPool; //un array de structuri Thread

int sd; //descriptorul de socket de ascultare
int nthreads;//numarul de threaduri
pthread_mutex_t mlock=PTHREAD_MUTEX_INITIALIZER;              // variabila mutex ce va fi partajata de threaduri


int main (int argc, char *argv[])
{
  struct sockaddr_in server;	// structura folosita de server  	
  void threadCreate(int);

   if(argc<2)
   {
        fprintf(stderr,"Eroare: Primul argument este numarul de fire de executie...");
	exit(1);
   }
   nthreads=atoi(argv[1]);
   if(nthreads <=0)
	{
        fprintf(stderr,"Eroare: Numar de fire invalid...");
	exit(1);
	}
    threadsPool = calloc(sizeof(Thread),nthreads);

   /* crearea unui socket */
  if ((sd = socket (AF_INET, SOCK_STREAM, 0)) == -1)
    {
      perror ("[server] Eroare la socket()");
      return errno;
    }
  /* utilizarea optiunii SO_REUSEADDR */
  int on=1;
  setsockopt(sd,SOL_SOCKET,SO_REUSEADDR,&on,sizeof(on));
  
  /* pregatirea structurilor de date */
  bzero (&server, sizeof (server));

  /* umplem structura folosita de server */
  /* stabilirea familiei de socket-uri */
    server.sin_family = AF_INET;	
  /* acceptam orice adresa */
    server.sin_addr.s_addr = htonl (INADDR_ANY);
  /* utilizam un port utilizator */
    server.sin_port = htons (PORT);

    FILE* tokens = fopen("valid_tokens.txt", "r+");
  
  /* atasam socketul */
  if (bind (sd, (struct sockaddr *) &server, sizeof (struct sockaddr)) == -1)
    {
      perror ("[server] Eroare la bind()");
      return errno;
    }

  /* punem serverul sa asculte daca vin clienti sa se conecteze */
  if (listen (sd, 2) == -1)
    {
      perror ("[server] Eroare la listen()");
      return errno;
    }

   printf("Nr threaduri %d \n", nthreads); fflush(stdout);
   int i;
   for(i=0; i<nthreads;i++) 
   	{
   		threadsPool[i].fdTokens=tokens;
   		threadCreate(i);
   	}

	
  /* servim in mod concurent clientii...folosind thread-uri */
  for ( ; ; ) 
  {
	printf ("[server] Asteptam la portul %d...\n",PORT);
        pause();				
  }
};
				
void threadCreate(int i)
{
	void *treat(void *);
	
	pthread_create(&threadsPool[i].idThread,NULL,&treat,(void*)i);
	return; /* threadul principal returneaza */
}

void decrease_attempts(FILE * tokens, char* line)
{
  fseek(tokens, ftell(tokens) - 1, SEEK_SET);
  fprintf(tokens, "%d", atoi(line+13)-1);
  rewind(tokens);
}

void sendFile(int client, char* fileName, int i)
{
  FILE *fp = fopen(fileName,"r");
  if(fp==NULL)
  {
    printf("File open error\n");
  }

  write(client, "Token acceptat!", 15);
  unsigned char buff[256]={0};
  int bytesRead=0;
  int sum=0;
  while((bytesRead = fread(buff,1,256,fp))>0)
  {
    sum=sum+bytesRead;
    /* Daca am citit ceva, trimitem la client */
    if(bytesRead > 0)
    {
      write(client, buff, bytesRead);
    }
  }
  printf("[thread %d] Sent %d bytes to client\n", i, sum);
}

/* realizeaza primirea si retrimiterea unui mesaj unui client */
int serveClient(int client, FILE* tokens, int i)
{
  char buffer[100];		/* mesajul */
  int bytes;			/* numarul de octeti cititi/scrisi */
  char token_read[13];
  char fileName[30];		 
  char msgrasp[100]=" ";        //mesaj de raspuns pentru client
  char line[40];

  bytes = read (client, buffer, sizeof (buffer));
  if (bytes < 0)
    {
      perror ("[thread] Eroare la read() de la client.\n");
      return 0;
    }

  strncpy(token_read, buffer, 12);
  strcpy(fileName, buffer+13);
  printf ("[thread %d] Received: %s - File req: %s\n", i, token_read, fileName);
  int found=0;
  while(fscanf(tokens, "%s", line) && !feof(tokens))
    {
      if(strstr(line, token_read)!=0 && atoi(line+13)>0)
      {
        printf("[thread %d] %s valid - Attempts remaining: %d\n",i,  token_read, atoi(line+13)-1);
        found=1;
        decrease_attempts(tokens, line);
        sendFile(client, fileName, i);
        break;
      }
    }
  if(found==0)
  {
    write(client, "Token expirat!", 14);
    rewind(tokens);
    return 0;
  }
  return 1;

}

void *treat(void * arg)
{		
		int client;
		        
		struct sockaddr_in from; 
 	        bzero (&from, sizeof (from));
 		printf ("[thread %d] Ready...\n", (int) arg);fflush(stdout);

		for( ; ; )
		{
			int length = sizeof (from);
			pthread_mutex_lock(&mlock);
			//printf("Thread %d trezit\n",(int)arg);
			if ( (client = accept (sd, (struct sockaddr *) &from, &length)) < 0)
				{
	 			 perror ("[thread] Eroare la accept()");	  			
				}
			pthread_mutex_unlock(&mlock);
			threadsPool[(int)arg].thCount++;

			while (serveClient(client, threadsPool[(int)arg].fdTokens, (int)arg))
        		{
        		 printf("==============================\n"); 
        		  
        		}
      printf ("[thread %d] Token expirat. Se deconecteaza clientul %d\n", (int)arg, client);
      printf("==============================\n");
      fflush (stdout);
      close (client);
			//raspunde(client,(int)arg); //procesarea cererii
			/* am terminat cu acest client, inchidem conexiunea */			
		}	
}



